...
# Function to backup and modify Steam's localconfig.vdf - CORRECTED VERSION
modify_steam_launch_options() {
    ...
    if pgrep -f "steam" > /dev/null 2>&1; then
        if ! show_question "⚠️ Steam is currently running!\n\nFor the changes to take effect properly, Steam should be closed.\n\nWould you like to continue anyway?\n\n(Changes will be applied when Steam restarts)"; then
            return 1
        fi
    fi
        ...
        if action == 'add':
            if app_match:
                ...
                if launch_match:
                    ...
                    # Remove existing LSFG and DXVK options
                    new_options = re.sub(r'DXVK_FRAME_RATE=\d+\s*', '', current_options)
                    new_options = re.sub(r'ENABLE_LSFG=\d+\s*', '', new_options)
                    new_options = re.sub(r'LSFG_MULTIPLIER=\d+\s*', '', new_options)
                    new_options = re.sub(r'LSFG_FLOW_SCALE=[\d.]+\s*', '', new_options)  # Matches any decimal
...
configure_lossless_scaling_for_game() {
    ...
    local choice=$(zenity --list \
        ...
        --width=600 \
        --height=320 \
        "Enable" "✅ Enable with recommended settings (2x, 0.5 flow)" \
...
enable_lossless_scaling_auto() {
    local app_id="$1"
    local game_name="$2"
    local lsfg_command="DXVK_FRAME_RATE=30 ENABLE_LSFG=1 LSFG_MULTIPLIER=2 LSFG_FLOW_SCALE=0.5 LSFG_PERF_MODE=1 %COMMAND%"
...
cconfigure_lossless_scaling_advanced_auto() {
    local app_id="$1"
    local game_name="$2"

    echo "Starting configure_lossless_scaling_advanced_auto for $game_name (ID: $app_id)"

    # Get DXVK frame rate limit
    local frame_rate=$(zenity --list \
        --title="DXVK Frame Rate Limit" \
        --text="Choose frame rate limit for $game_name:\n\nDXVK_FRAME_RATE limits the maximum FPS" \
        --column="Frame Rate" \
        --column="Description" \
        --width=450 \
        --height=350 \
        "0" "No limit (disabled)" \
        "24" "24 FPS (cinematic)" \
        "30" "30 FPS (console standard)" \
        "40" "40 FPS (Steam Deck balanced)" \
        "50" "50 FPS (smooth gaming)" \
        "60" "60 FPS (standard PC)" \
        "75" "75 FPS (high refresh)" \
        "90" "90 FPS (high performance)" \
        "120" "120 FPS (competitive)" \
        "Custom" "Enter custom value" \
        2>/dev/null)

    if [ -z "$frame_rate" ]; then
        echo "User cancelled frame rate selection"
        return
    fi

    # Handle custom frame rate
    if [ "$frame_rate" = "Custom" ]; then
        frame_rate=$(zenity --entry \
            --title="Custom Frame Rate" \
            --text="Enter custom frame rate (0-240):" \
            --entry-text="60" \
            2>/dev/null)
        
        if [ -z "$frame_rate" ]; then
            echo "User cancelled custom frame rate"
            return
        fi
        
        # Validate frame rate is a number between 0 and 240
        if ! [[ "$frame_rate" =~ ^[0-9]+$ ]] || [ "$frame_rate" -lt 0 ] || [ "$frame_rate" -gt 240 ]; then
            show_error "❌ Invalid frame rate!\n\nPlease enter a number between 0 and 240."
            return
        fi
    fi

    echo "Selected frame rate: $frame_rate"
    ...
    # Build the command with DXVK_FRAME_RATE
    local lsfg_command=""
    if [ "$frame_rate" != "0" ]; then
        lsfg_command="DXVK_FRAME_RATE=$frame_rate ENABLE_LSFG=1 LSFG_MULTIPLIER=$multiplier LSFG_FLOW_SCALE=$flow_scale LSFG_PERF_MODE=$perf_mode %COMMAND%"
    else
        lsfg_command="ENABLE_LSFG=1 LSFG_MULTIPLIER=$multiplier LSFG_FLOW_SCALE=$flow_scale LSFG_PERF_MODE=$perf_mode %COMMAND%"
    fi
...
4. In 'Launch Options' field, add:
   DXVK_FRAME_RATE=30 ENABLE_LSFG=1 LSFG_MULTIPLIER=2 LSFG_FLOW_SCALE=0.50 LSFG_PERF_MODE=1 %COMMAND%

🔧 PARAMETERS:

DXVK_FRAME_RATE=X
   • Limits maximum frame rate (0 = no limit)
   • Recommended: 30 for Steam Deck, 60 for PC
...
PC Standard (60 FPS):
DXVK_FRAME_RATE=60 ENABLE_LSFG=1 LSFG_MULTIPLIER=2 LSFG_FLOW_SCALE=0.50 LSFG_PERF_MODE=1 %COMMAND%

Ultra Smooth (no frame limit):
ENABLE_LSFG=1 LSFG_MULTIPLIER=2 LSFG_FLOW_SCALE=0.25 LSFG_PERF_MODE=1 %COMMAND%

Competitive Gaming:
DXVK_FRAME_RATE=120 ENABLE_LSFG=1 LSFG_MULTIPLIER=2 LSFG_FLOW_SCALE=0.80 LSFG_PERF_MODE=1 %COMMAND%

⚠️  NOTES:
- Lossless Scaling must be installed separately
- DXVK_FRAME_RATE works with Vulkan/DXVK games
